...
|---|---|---|
| **1.2.0** | **1.1.7** 及以上 | 2024.12 及以上 |
| 1.1.x | 1.1.0 及以上 | 2024.12 及以上 |
...
播放组实体的行为与 Sonos 分组一致：

- 组实体在 `group_members` 中排在首位，充当 leader。
- 成员入组后，它的传输控制会转发给 `group:<id>`，播放/暂停/队列操作作用于整组。
- **音量仍然是每设备独立的**，可以单独平衡各个音箱。
- 在 HA 卡片的分组界面操作，会真实地在服务端创建、修改或解散对应的 MusicFlow 播放组。
